...
def setServoPulse(address, index, pos):	
		logi.directWrite(address+(index * 2), ((pos & 0x00FF), ((pos >> 8) & 0x00FF)));
...
def setServoFailSafeAngle(address, index, angle, max_pulse=255.0, min_angle=-45.0, max_angle=45.0):
	...
	logi.directWrite(address+((index * 2) + 1), ((pulse & 0x00FF), ((pulse >> 8) & 0x00FF)));
...
def setPWMDivider(address, divider):	
		logi.directWrite(address, ((divider & 0x00FF), ((divider >> 8) & 0x00FF)));

def setPWMPeriod(address, period):	
		logi.directWrite(address+1, ((period & 0x00FF), ((period >> 8) & 0x00FF)));

def setPWMPulse(address, chan, pulse):	
		logi.directWrite(((address+2)+chan), ((pulse & 0x00FF), ((pulse >> 8) & 0x00FF)));

def setGPIODir(address, dir_mask):	
		logi.directWrite(address+1, ((dir_mask & 0x00FF), ((dir_mask >> 8) & 0x00FF)));

def setGPIOVal(address, val):	
		logi.directWrite(address, ((val & 0x00FF), ((val >> 8) & 0x00FF)));

def getGPIOVal(address):	
		read_val = logi.directRead(address, 2);
		return read_val[0]+(read_val[1] << 8)

def enableWatchdog(address):	
		logi.directWrite(address, (0x01, 0x00) );

def disableWatchdog(address):	
		logi.directWrite(address, (0x00, 0x00) );
